...
def printHelp() {
    log.info """
    Selenoprotein Hunter Pipeline with integrated geneid training
    =============
    
    Usage:
        nextflow run main_model.nf --species_taxid <taxid> --species_name <name> --annotation_url <url> --fasta_url <url> [options]
...
include { CLEAN_GTF as CLEAN_SPLIT_GTF } from './modules/clean_gtf'
...
include { CREATE_PARAM_FILE } from './modules/create_param_file'
...
workflow {
    ...
    // Step 2: Run AGAT_GFF2GTF in parallel to standardise each GFF file 
    ...
    // Create a param file 
    param_file = CREATE_PARAM_FILE(downloaded_files.fasta, cleaned_gtf_ch.cleaned_gtf, params.species_name)
    ...
    relocated_gtf = RELOCATE_TRANSCRIPTS(final_cleaned_gtf_ch.cleaned_gtf).relocated_gtf
    relocated_gtf_val = relocated_gtf.first()
    
    // Step 7. Recode all transcripts 
    recoded_transcripts = RECODE_TGA(gffread_outputs.transcripts)

    // Step 8. Split recoded transcripts if too large
    split_transcripts_ch = SPLIT_IF_TOO_LARGE(recoded_transcripts).split_fasta.flatten()
     
    // Step 9: Run geneid on all the transcript sequences 
    geneid_results_ch = RUN_GENEID_ORIGINAL(split_transcripts_ch, param_file.param_file).geneid_results 
